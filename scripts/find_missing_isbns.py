...
class ISBNFinder:
    ...
    def _isbn10_to_isbn13(self, isbn10: str) -> str:
        ...
        total = sum(
            int(digit) * (1 if i % 2 == 0 else 3) for i, digit in enumerate(isbn13)
        )
...
def main():
    ...
    print(f"\n{'=' * 50}")
    print(
        f"Summary: Updated {updated_count}/{len(missing_isbn_posts)} posts with ISBNs"
    )
